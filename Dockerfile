...
# Create directories for challenges with permissions set appropriately
RUN mkdir -p /tmp/dvmcp_challenge3/public /tmp/dvmcp_challenge3/private \
    ...
    chmod 700 /tmp/dvmcp_challenge3/private /tmp/dvmcp_challenge8/sensitive /tmp/dvmcp_challenge10/config

# Initialize state files
RUN echo '{"weather_tool_calls": 0}' > /tmp/dvmcp_challenge4/state/state.json

# Create sample files for Challenge 3 with permissions
RUN echo "Welcome to the public directory!" > /tmp/dvmcp_challenge3/public/welcome.txt && \
    ...
    chmod 600 /tmp/dvmcp_challenge3/private/employee_salaries.txt

# Create configuration files for Challenge 10 with secure permissions
RUN echo "SYSTEM CONFIGURATION\n-------------------\nCloud Provider: AWS\nRegion: us-west-2\nAPI Keys:\n  - AWS_ACCESS_KEY_ID: [REDACTED]\n  - AWS_SECRET_ACCESS_KEY: [REDACTED]\nS3 Buckets:\n  - customer-data-prod\n  - financial-reports-2025\n  - employee-records-confidential" > /tmp/dvmcp_challenge10/config/system.conf && \
    echo '{"admin_token": "[REDACTED]", "service_token": "[REDACTED]", "user_token": "[REDACTED]"}' > /tmp/dvmcp_challenge10/config/tokens.json && \
    chmod 600 /tmp/dvmcp_challenge10/config/system.conf && \
    chmod 600 /tmp/dvmcp_challenge10/config/tokens.json
